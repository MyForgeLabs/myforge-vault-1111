...
# ----------------------------------------------------------------------
# page-in — real archival snippet retrieval (MemGPT page-fault)
# ----------------------------------------------------------------------


def _fetch_chunk_text(file: str, chunk_idx: int, namespace: str = "content") -> str:
    """Pull full chunk text from Memgraph for (file, chunk_idx, namespace)."""
    try:
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "MATCH (c:Chunk {file: $file, chunk_idx: $idx, namespace: $ns}) "
            "RETURN c.text AS text LIMIT 1",
            {"file": file, "idx": chunk_idx, "ns": namespace},
        )
        row = cur.fetchone()
        conn.close()
...
def cmd_page_in(args: argparse.Namespace) -> int:
    """Pull archival snippets for a query — the MemGPT page-fault primitive.

    Unlike `simulate` (which only counts tokens), `page-in` returns the
    actual chunk text in a markdown block the agent can paste into its
    context. This is the runtime workhorse of the virtual-context pattern.
    """
    query = args.query
    top_k = args.top_k
    ...
    # Run vault-search hybrid for ranking
    try:
        proc = subprocess.run(
            ["vault-search", query, "--top-k", str(top_k),
             "--namespace", namespace, "--hybrid", "--json"],
            capture_output=True, text=True, timeout=30,
        )
        if proc.returncode != 0:
            print(f"✗ vault-search exit {proc.returncode}: "
                  f"{proc.stderr.strip()[:200]}", file=sys.stderr)
            return 1
        data = json.loads(proc.stdout)
    except (FileNotFoundError, subprocess.TimeoutExpired,
            json.JSONDecodeError) as exc:
        print(f"✗ vault-search unavailable: {exc}", file=sys.stderr)
        return 1

    results = data.get("results", []) if isinstance(data, dict) else []
    if not results:
        if args.format == "json":
            print(json.dumps({"query": query, "results": [], "tokens": 0},
                             ensure_ascii=False, indent=2))
        else:
            print(f"# Archival page-in for {query!r}\n\n(no results)")
        return 0

    # Resolve full chunk text per result (Memgraph), cap to max_chars per chunk
    enriched = []
    per_chunk_cap = max_chars // max(1, len(results))
    for r in results[:top_k]:
        file = r.get("file", "")
        chunk_idx = r.get("chunk_idx", 0)
        snippet = r.get("snippet", "") or ""
        full = _fetch_chunk_text(file, chunk_idx, namespace) or snippet
        text = full[:per_chunk_cap]
        enriched.append({
            "file": file,
            "chunk_idx": chunk_idx,
            "title": r.get("title", ""),
    ...
    total_tokens = sum(e["text_tokens"] for e in enriched)
    ...
    # Markdown — agent-paste-ready format
    lines = [
        f"# Archival page-in for {query!r}",
        "",
        f"> Retrieved {len(enriched)} chunks (~{total_tokens} tokens) "
        f"via `vault-search --hybrid`. Cap: {max_chars} chars total, "
        f"~{per_chunk_cap} chars per chunk.",
        "",
    ]
    for i, e in enumerate(enriched, 1):
        title = e["title"] or e["file"]
        lines.append(f"## [{i}] {title}")
        lines.append("")
        lines.append(f"_source: `{e['file']}` chunk {e['chunk_idx']} "
                     f"· ~{e['text_tokens']} tokens_")
        lines.append("")
        lines.append(e["text"])
        lines.append("")
    sys.stdout.write("\n".join(lines))
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_page = sub.add_parser(
        "page-in",
        help="Retrieve archival snippets for a query (MemGPT page-fault).",
    )
    p_page.add_argument("query")
    p_page.add_argument("--top-k", type=int, default=5,
                        help="how many chunks to retrieve (default 5)")
    p_page.add_argument("--max-chars", type=int, default=4000,
                        help="total char-budget across all chunks (default 4000)")
    p_page.add_argument("--namespace", default="content",
                        help="Memgraph chunk namespace (default 'content')")
    p_page.add_argument("--format", choices=["markdown", "json"],
                        default="markdown", help="output format")
    p_page.set_defaults(func=cmd_page_in)

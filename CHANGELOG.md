...
- **Cron mutex-coverage 88% → 100%**: 4 unprotected entries (`-notify`, `github-trending-report`, `backup.sh`, `notebooklm keepalive`) wrapped in `flock -n`. Also corrected `vault-doctor`'s detection to recognize both `vault-cron-flock` wrapper AND direct `flock -n` / `flock -w` as mutex-protected (was undercounting).

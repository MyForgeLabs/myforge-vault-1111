...
# These watch for things that must NEVER be allowed in any commit:
#   - plugin-config with confirmation-bypass instruction-injection hooks
# Each guard self-decides whether to fire based on the staged file-set.
# ---------------------------------------------------------------------------
PLUGIN_HOOKS_WATCH="$(git rev-parse --show-toplevel 2>/dev/null || echo /root/obsidian-vault)/.git/hooks/pre-commit-plugin-hooks-watch.sh"

...
## 2026-05-20 — Sandbox-branch subset for DB-write sprints

A 4-rétegű safety-gate **high-risk RSI / auto-mutate** workflow-ra van. **DB-write-only sprint-ekre** (KO-DB ingest, Memgraph push) egy kisebb subset is elég, mert a "veszély" adat-szennyezés, NEM kód-mutáció.

**Sandbox-branch subset pattern**:

1. **`git checkout -b <sprint>-sandbox`** — minden DB-write művelet sandbox-branchen él. Main marad tiszta.
2. **DB-backup file** `<db>.pre-<sprint>-<date>.bak` — single-shot revert garancia.
3. **Process-ready idempotency** — minden DB-write művelet repeatable (SCD2 + UNIQUE(hash) verified 0 collision).
4. **Per-batch measurement + user-decision-checkpoint** — minden 20-30% progress után.
5. **`git merge --no-ff` to main** csak miután final-measurement bizonyítja az acceptance criteria-t.

**Élő példa — 2026-05-20 Tier-1 backfill 171/171**: `treesitter-prepass-sandbox` branch, 2 DB-backup, 11 process-ready cycle (0 collision), 2 checkpoint, merge 27e5e21 csak miután FCA 0.93 + XR_T1 1.0 + XR_T2 0.90 verified.

**Wider lesson**: **match the gate to the risk profile**. RSI/auto-mutate → full 4-layer. DB-write-only → sandbox + backup + idempotent + checkpoint elég. Code-only-change → simple PR review.

## Layer-0 chained guards pattern (2026-05-21)

A `bmad-vault-bridge --apply` safety-gate-en TÚL (ami csak `CRYSTALLIZE_APPLYING=1` módban fut), egy új réteg landolt: **`Layer 0 always-on chained guards`** — minden commit-on fut, file-pattern-trigger-rel.

### A minta

A fő pre-commit hook (`.vault-ko/safety/git-pre-commit-hook.sh`) tetején, MINDEN egyéb gate ELŐTT:

```bash
# Layer 0 — always-on chained guards (run regardless of crystallize mode)
PLUGIN_HOOKS_WATCH="$(git rev-parse --show-toplevel)/.git/hooks/pre-commit-plugin-hooks-watch.sh"
if [[ -x "$PLUGIN_HOOKS_WATCH" ]]; then
  if ! bash "$PLUGIN_HOOKS_WATCH"; then
    exit $?
  fi
fi
```

A chained guard maga **self-decides** hogy fire-ol vagy nem, a staging-listán lévő file-pattern-ek alapján. Ha nincs releváns staging → exit 0 (no-op). Ha van → futtatja a megfelelő audit-CLI-t `--strict` módban.

### Trigger-pattern + audit párosítások

| Staged file pattern | Audit CLI | Block ha |
|---|---|---|
| `*hooks.json`, `.claude/*`, `.codex/*`, `.gemini/*`, `*/settings.json` | `vault-plugin-hooks-audit --strict` | HIGH-heat instruction-injection pattern (lásd [[claude-code-harness-blocks]] § 7) |
| `*.mcp.json`, `*/mcp.json` | `vault-mcp-audit --strict` | HIGH-heat MCP risk-vector (shell-exec / non-HTTPS / literal-cred — lásd [[mcp-server-safety-classification]]) |
| `*.sql`, `migrate-*.py` | `vault-schema-migration-victim-audit --strict` | New schema-change ADR + unpatched downstream-callers |

### Miért külön réteg

A `Layer 0` szándékosan **always-on** és **crystallize-mode-független**:

- A `Layer 3` forbidden-target deny-list csak `CRYSTALLIZE_APPLYING=1`-nél fut (intentional — human edits + autosave NE legyenek blokkolva)
- A `Layer 0` viszont **bármilyen commit-on** fut — kézi git commit, autosave-cron, pre-commit-from-other-script. Akkor is, ha NEM crystallize-flow van.
- A trigger maga file-pattern-trigger, NEM env-var-trigger — így nem lehet "kifelejteni" beállítani egy env-var-t és bypass-elni.

### Hozzáadás új audit-tengelyhez

A chain lineárisan skálázódik. Új audit-class hozzáadása:

1. Hozz létre `vault-<class>-audit` CLI-t `--strict` flag-gel
2. Egészítsd ki a `pre-commit-plugin-hooks-watch.sh`-t egy új `if [[ TRIGGER_<class> ... ]] then ... fi` blokkal
3. Trigger-pattern: a class-specifikus file-glob
4. Override env-var: `SKIP_<CLASS>_AUDIT=1 git commit ...` (emergency-only, audit-trail via shell history)

A pattern jelenleg `hooks.json` + `.mcp.json` triggert kezel; várakozó class-ok: `*.sql` schema-changes, `Dockerfile` permission-changes, `requirements.txt` package-pin-changes.

### Audit-cron alá-rétegként

Minden Layer-0 guard mögött áll **egy heti audit-cron** ami a teljes vault-on (NEM csak a staged-on) fut és `06-Audits/<class>-audit-YYYY-WNN.md` baseline-t ír. A Layer-0 a "**don't add new HIGH-heat**" gate, a heti cron a "**no HIGH-heat in production**" monitor. Két különböző use-case, ugyanaz a CLI hajtja meg mindkettőt.
...
- [[rsi-tier2-constitutional-ai-pattern]] — RSI safety-gate konkrét megvalósítás (4-rétegű implementation)
- [[subagent-fanout-for-planning-artifact.en]] — sibling pattern: planning-artifact also safety-discipline-applicable
- [[../06-Audits/2026-05-20 Tier-1 backfill COMPLETE 171-171 — FCA 0.93 (ceiling reached)]] — sandbox-branch subset pattern élő példa

...
## Orchestrator pattern: per-axis cron + weekly rollup (2026-05-21)

A continuous-eval architektúra **két különböző réteget** kombinál — a kettő nem helyettesíti, hanem **kiegészíti** egymást:

### Réteg 1 — Per-axis cron (atomic measurements)

Minden eval-tengely saját cron-job-bal mér, saját idő-ablakkal, saját output-fájlra:

| Cron | Time | What | Output |
|---|---|---|---|
| `vault-eval-regression` | Daily 02:45 | LongMemEval-S R@5 retrieval baseline | `06-Audits/vault-eval-regression.json` |
| `vault-graph-diff` | Sun 05:00 | Jaccard label-overlap (legacy reference) | `06-Audits/vault-graph-diff-weekly.json` |
| `vault-graph-extract --reset` | Sun 03:30 | Memgraph clean rebuild | (Memgraph state) |
| `vault-graph-complementarity` | Sun 05:30 | FCA / CD / XR two-tier metrics | `06-Audits/YYYY-MM-DD two-tier complementarity baseline.md` |
| `vault-ko-conflicts-audit` | Sun 04:30 | Cross-source contradictions | `06-Audits/cross-source-conflicts-YYYY-WNN.md` |
| `vault-ko-belief-weekly` | Sun 04:45 | Bayesian belief-update health | `06-Audits/ko-belief-weekly-YYYY-WNN.md` |
| `vault-plugin-hooks-audit` | Mon 05:30 | Marketplace plugin instruction-injection | `06-Audits/plugin-hooks-audit-YYYY-WNN.md` |
| `vault-mcp-audit` | Mon 05:45 | MCP-server registration safety | `06-Audits/mcp-audit-YYYY-WNN.md` |

Each is **atomic** — a single measurement with its own retry / error / freshness story. They write structured output (JSON or frontmatter+markdown) but **don't talk to each other**.

### Réteg 2 — Weekly rollup orchestrator

`vault-continuous-eval` (Sunday 06:00 UTC, **after** all per-axis cron jobs):

- Reads the latest output of each per-axis cron
- Extracts key metrics from each (regex/JSON-parse, tolerant of format variations)
- Diffs vs the previous week's rollup (`continuous-eval-YYYY-WNN.md`)
- Heat-classifies each axis (🟢 ok / 🟡 warn / 🔴 alert / ⏳ stale / ⚪ missing)
- Writes a **single markdown summary** + machine-readable JSON snapshot embedded
- `--strict` mode exits 1 if any axis is in alert state (for notification pipelines)

### Why the orchestrator matters

**Per-axis cron alone is necessary but not sufficient.** Without aggregation:

- Drift across axes is invisible (e.g. plugin-hooks newly red AND complementarity newly yellow = correlated event)
- Week-over-week deltas exist but require manual grepping
- A single "is everything OK?" question has no answer below the 8-tab-of-grep-output level
- Human-reviewer attention drops sharply after ~4 simultaneous dashboards

The orchestrator fixes all 4 by collapsing into a **single-pane-of-glass weekly rollup**.

### Generalization beyond this vault

This pattern applies to any multi-axis monitoring system: **never settle for "each axis has a cron"**. Always add the weekly aggregator. The marginal cost (1-2h to build the rollup) is dwarfed by the downstream cost of missing correlated drift.

Implementation: [[../06-Audits/continuous-eval-2026-W22|first baseline rollup]] · CLI source: `/root/obsidian-vault/.vault-tools/scripts/vault-continuous-eval.py`.

...
CE-ban nincs `DATABASE` koncepció — minden 1 alapértelmezett `memgraph` DB-ben él. Vault-megoldás: **label-prefix namespacing** (`:SV_Entity` vs `:Foxxi_Entity`), vagy külön Docker-container per projekt (`vault-memgraph`, `client-b-memgraph`, eltérő portokon).

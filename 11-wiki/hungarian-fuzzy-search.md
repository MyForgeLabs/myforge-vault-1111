...
- **300-3000 elemű** in-memory dataset (KGC: 292 gép, ~1ms / query / elem)
...
A `kgc-berles/app/api/search/route.ts` self-contained — emelhető `lib/fuzzy-hu.ts`-be: 3 export (`normalize`, `levenshtein`, `scoreMatch`). Foxxi keresőhöz, example-balance.local instrumentum-keresőhöz is jó.

## Forrás

- Implementáció: `/root/projektjeim/KGC-ALL/kgc-berles/app/api/search/route.ts`

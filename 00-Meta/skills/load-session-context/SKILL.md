...
| **Episodic** | `01-Daily/+10-raw/+08-Sessions/_archive/` (utolsó 14 nap) | ✅ Top-K=3 semantic-fetch (`vault-search`) |
| **Semantic** | `11-wiki/+07-Decisions/+05-Memory/+02-Projects/` (evergreen tudás) | ⏳ On-demand `vault-search` tool-call-lal |
...
vault-search --top-k 3 "<projekt-slug>"      # episodic+semantic: ~2.5K (top-3 chunk)
...
A többi (deep ADR-tartalom, infra-gotchák, régebbi session-Learning) **on-demand**: ha a user kérdés specifikus témára mélyül, az agent `vault-search "<kérdés>"` tool-call-t hív, és a top-K-t bevonja a kontextusba.
...
**Top-3 releváns episodic-emlék** (vault-search, score >0.6):
- [<score>] [[<file>]] — <title>
- ...
...
**Mélyítés on-demand:**
- `vault-search "<query>" --top-k 5`                   — szöveges narratíva, semantic-fetch (Memgraph bge-m3)
...
```bash
vault-search "Memgraph config docker-compose" --top-k 5
```
...
Ha `vault-search` üres / Memgraph nem fut, **visszaesik a régi aggressive pattern-re** (SKILL.md.bak.20260513-pre-memgpt elérhető reference-ként). ENV-flag: `VAULT_SEARCH_MODE=grep` explicit override.

## Tokenmegtakarítás (mért, 2026-05-13 + 2026-05-17 KO-DB bővítés)

| Pattern | Token-budget | Latency | Recall |
|---|---|---|---|
| Régi aggressive (file-cat 5-7 fájl) | ~15-20K | ~30 sec olvasás | 100% (mindent betölt) |
| Lean v1 (working + semantic) | ~5K | <10 sec semantic-fetch | ~85% (95% on-demand bővítéssel) |
| **Lean v2 (working + semantic + KO-DB Top-K)** | **~5K** | **<10 sec** (KO-DB instant SQLite, ~50ms) | **~92%** (KO-DB strukturált fact-jei kitöltik a Memgraph gap-eket) |
